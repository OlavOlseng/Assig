...
public class Process implements Constants
{
     ...
     */
    public void leaveMemoryQueue(long clock) {
	...
	public void updateStatistics(Statistics statistics) {
		...
		statistics.totalTimeSpentProcessing += timeSpentInCpu;
		statistics.totalTimeSpentInIO += timeSpentInIo;
		statistics.totalEntriesInCPUqueue += nofTimesInReadyQueue;
		statistics.totalEntriesInIOqueue += nofTimesInIoQueue;
	...
	public void leaveCPU(long clock) {
		...
		if(processId == 1) System.out.println("TISS: " + timeSpentInCpu);
		timeOfLastEvent = clock;
	}
	
	public void leaveReadyQueue(long clock){
	...
	public void leaveSystem(long clock) {
		//TODO: implement
	}
	
	/***
	 * Will return true if the process will need IO during its frame.
	 * @param clock
	 * @return
	 */
	public boolean needsIoInInterval(long clock, long CPUFrameSize) {
		//Check if process will need IO during its time in the processor
		if(CPUFrameSize >= this.timeToNextIoOperation) {
			return true;
		}
		return false;
	}
