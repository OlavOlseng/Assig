...
glm::mat4 MVP1,MVP2,MVP3,MVP4,MVP5,MVP6, MVP7, MODEL_EVERYTHING, MODEL_LEG_1, MODEL_LEG_2, MODEL_FOOT_1, MODEL_FOOT_2;
...
void RenderScene5()
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, footbuffer);
	glVertexAttribPointer(
		0,                  // attribute. No particular reason for 0, but must match the layout in the shader.
		3,                  // size
		GL_FLOAT,           // type
...
void RenderScene6() 
{
	...
	View = glm::lookAt(
			position,
			glm::vec3(MODEL_EVERYTHING[3][0], MODEL_EVERYTHING[3][1], MODEL_EVERYTHING[3][2]),
			up
			);
...
void SetupGL() //
{
	...
	programID_2 = LoadShaders("TransformVShader2.vertexshader", "ColorFragmentShader2.fragmentshader");
	MatrixID = glGetUniformLocation(programID_1, "MVP");
	colorID = glGetUniformLocation(programID_2, "colorIN");
	...
	static const GLfloat g_foot_buffer_data[] = {
		//upper faces
		0.0f, 0.25f, 0.0f,
		0.0f, -0.25f, -1.0f,
		-0.3f, -0.25f, 0.0f,
	...
	glGenBuffers(1, &footbuffer);
	glBindBuffer(GL_ARRAY_BUFFER, footbuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(g_foot_buffer_data), g_foot_buffer_data, GL_STATIC_DRAW);
